...
function addFormEvent(formid, focus){
	...
	formNode[stmp[0]].onblur = function () {
		...
		{
			var unmsg='<img src="template/we54/common/images/register_no.png" />用户名不能为空';
	...
	formNode[stmp[1]].onblur = function () {
		if(formNode[stmp[1]].value == '') {
			var pwmsg = '<img src="template/we54/common/images/register_no.png" />请填写密码';
			if(pwlength > 0) {
	...
	formNode[stmp[2]].onblur = function () {
		if(formNode[stmp[2]].value == '') {
			errormessage(formNode[stmp[2]].id, '<img src="template/we54/common/images/register_no.png" />请再次输入密码');
		}
		checkpassword(formNode[stmp[1]].id, formNode[stmp[2]].id);
	};
...
function addMailEvent(mailObj) {
	...
	mailObj.onblur = function () {
		if(mailObj.value == '') {
			errormessage(mailObj.id, '<img src="template/we54/common/images/register_no.png" />请输入邮箱地址');
		}
...
function checkusername(id) {
	...
	if(username.match(/<|"/ig)) {
		errormessage(id, '<img src="template/we54/common/images/register_no.png" />用户名包含敏感字符');
		return;
	}
	var unlen = username.replace(/[^\x00-\xff]/g, "**").length;
	if(unlen < 3 || unlen > 15) {
		errormessage(id, unlen < 3 ? '<img src="template/we54/common/images/register_no.png" />用户名不得小于 3 个字符' : '<img src="template/we54/common/images/register_no.png" />用户名不得超过 15 个字符');
		return;
	}
	var x = new Ajax();
	$('tip_' + id).parentNode.className = $('tip_' + id).parentNode.className.replace(/ p_right/, '');
...
function checkpassword(id1, id2) {
	...
	errormessage(id2);
	if($(id1).value != $(id2).value) {
		errormessage(id2, '<img src="template/we54/common/images/register_no.png" />两次输入的密码不一致');
	} else {
...
function checkemail(id) {
	...
	if(email.match(/<|"/ig)) {
		errormessage(id, '<img src="template/we54/common/images/register_no.png" />Email 包含敏感字符');
		return;
	}
	var x = new Ajax();
	$('tip_' + id).parentNode.className = $('tip_' + id).parentNode.className.replace(/ p_right/, '');
...
function checknickname(id) {
	...
	if(nickname.match(/<|"/ig)) {
		errormessage(id, '<img src="template/we54/common/images/register_no.png" />社区昵称包含敏感字符');
		return;
	}
