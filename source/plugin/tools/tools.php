 ...
 *
 *      $Id: tools.php 111 2012-06-15 02:43:43Z fuqingrong $
 */

/**
 * 密码要求：1、必须且只能包含大写字母、小写字母、数字
 *           2、密码长度大于6
 * 修改下面的 $password = ''，单引号中按照密码要求写入你的密码，举例 $password = 'Comsenz2012' ，注意：请不要把密码设置成 Comsenz2012，以免被人获知。
 */
$tpassword = '';
...
define('TOOLS_VERSION', 'Tools 2.5.0');
define('TOOLS_DISCUZ_VERSION', 'Discuz! X2.5');
...
END;
		...
		//修复数据库
		$t->connect_db();
		$typearray = array('index', 'repair', 'allrepair', 'allcheck', 'check', 'detail');
...
END;
		} elseif ($type == 'allrepair' || $type == 'allcheck' || $type == 'detail' || $type == 'check' || $type == 'repair') {
...
END;
				foreach($tablelists as $name => $value) {
					if($value['size'] < 1024) {
						echo '<tr><th>'.$name.'</th><td style="text-align:right;color:#339900"">'.$value['size'] .'KB</td><td>';
					} elseif(1024 < $value['size'] && $value['size']< 1048576 ) {
						echo '<tr><th>'.$name.'</th><td style="text-align:right;color:#3333FF">'.round($value['size']/1024,1) .'MB</td><td>';
					} elseif(1048576 < $value['size']){
						echo '<tr><th>'.$name.'</th><td style="text-align:right;color:#FF0000"">'.round($value['size']/1048576,1) .'GB</td><td>';
					}

					if(!isset($value['statu'])) {
						echo "<button class=\"pn vm\" onclick=\"jumpurl('tools.php?action=repairdb&type=check&table=".$name."')\"><strong>检查</button>";
					...
					if($value['statu']!='OK' && $value['statu']!='Not Support CHECK') {
						echo "<button class=\"pn vm\" onclick=\"jumpurl('tools.php?action=repairdb&type=repair&table=".$name."')\"><strong>修复</button></strong>";
					}
					echo '</td></tr>';
					}
...
function checkpassword($password) {
	$errormsg = array(	
						...
						6 => '没有设置密码，请使用FTP或者直接编辑论坛根目录下的 source/plugin/tools/tools.php 文件，并根据文件中的说明设置密码',
						7 => '不能使用密码示范中的的 Comsenz2012 为密码',
					);
	if(empty($password))
		return $errormsg[6];
...
function show_footer() {
	...
	<div id="footer">Powered by {$tools_versions} &copy; Comsenz Inc. 2001-2012 http://www.comsenz.com</div>
	</div>
	<br>
	</body>
	</html>
