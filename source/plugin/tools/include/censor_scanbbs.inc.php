...
if(submitcheck('bbsscansubmit',1)) {
    ...
    if(DB::result_first("SELECT count(pid) from ".DB::table($posttable)." where pid >= $start and pid <= $end AND invisible = 0 $sqlplus") == 0 && $scantype == 'addscan' && ($posttableid < 1)) {
    	cpmsg($toolslang[censor_noneedtoscan], "action=plugins&cp=censor_scanbbs&pmod=safe&operation=$operation&do=$do&identifier=$identifier&a=scanbbs", 'succeed');
    	...
    	cpmsg($toolslang[censor_jumpinto], "action=plugins&cp=censor_scanbbs&pmod=safe&operation=$operation&do=$do&identifier=$identifier&a=scanbbs&posttableid=$posttableid2&scantype=addscan&bbsscansubmit=yes", 'loading',array('id' => $posttableid));
    }
