...
while ($row = $db_source->fetch_array($query)) {
	...
	$db_target->query("REPLACE INTO $table_target SET tableid='$tableid',aid='$row[aid]',tid='$row[tid]',pid='$row[pid]',uid='$row[uid]',downloads='$row[downloads]'");
	...
	if(($row['isimage'] == '1' || $row['isimage'] == '-1') && $row['attachment']) {
		$queryb = $db_target->query("SELECT * from $table_target_forum_thread WHERE tid='$row[tid]'");
		while ($rownew = $db_target->fetch_array($queryb)) {
